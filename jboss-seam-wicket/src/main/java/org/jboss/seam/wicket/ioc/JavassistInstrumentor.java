...
public class JavassistInstrumentor implements ClassFileTransformer {
	...
	*/
	public void instrumentClass(CtClass implementation) throws NotFoundException, CannotCompileException {
		...
		CtMethod getEnclosingInstance = CtNewMethod.make(
				"public " + InstrumentedComponent.class.getName()
						+ " getEnclosingInstance() { return getHandler() == null ? null : getHandler().getEnclosingInstance(this); }",
				implementation);
	...
	*/
	private static String createBody(CtClass clazz, CtMethod method, CtMethod newMethod) throws NotFoundException {
		String src = "{" + createMethodObject(clazz, method) + "if (getHandler() != null) getHandler().beforeInvoke(this, method);"
	...
	*/
	private static String wrapInExceptionHandler(String src) {
		return "try {" + src
				+ "} catch (Exception e) { throw new RuntimeException(getHandler() == null ? e : getHandler().handleException(this, method, e)); }";
	}
	...
	*/
	public boolean markedInstrumentable(CtClass clazz) {
		...
		try {
			CtClass enclosing = Modifier.isStatic(clazz.getModifiers()) ? null : clazz.getDeclaringClass();
