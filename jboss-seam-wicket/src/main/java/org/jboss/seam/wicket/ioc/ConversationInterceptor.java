...
public class ConversationInterceptor<T> implements StatelessInterceptor<T> {
	...
	private boolean isMissingJoin(InvocationContext invocationContext) {
		return Manager.instance().isLongRunningOrNestedConversation() && ((invocationContext.getAccessibleObject()
				.isAnnotationPresent(Begin.class) && !invocationContext.getAccessibleObject().getAnnotation(Begin.class).join()
	...
	private void checkForConversation(InvocationContext<T> invocationContext) {
		if (!Manager.instance().isLongRunningConversation()
			...
			if (noConversationPage != null) {
				...
				throw new NoConversationException("no long-running conversation for @Conversational wicket component: "
	...
	private void beginConversationIfNecessary(InvocationContext invocationContext, Object result) {
		...
		boolean simpleBegin = invocationContext.getAccessibleObject().isAnnotationPresent(StartTask.class)
				|| invocationContext.getAccessibleObject().isAnnotationPresent(BeginTask.class)
				|| invocationContext.getAccessibleObject().isAnnotationPresent(org.jboss.seam.wicket.annotations.Begin.class)
				|| (invocationContext.getAccessibleObject().isAnnotationPresent(Begin.class)
						&& invocationContext.getAccessibleObject().getAnnotation(Begin.class).ifOutcome().length == 0);
		if (simpleBegin) {
			if (result != null
					|| (invocationContext.getMethod() != null && invocationContext.getMethod().getReturnType().equals(void.class))
					|| invocationContext.getConstructor() != null) {
			...
			if (outcomes.length == 0 || Arrays.asList(outcomes).contains(result)) {
				beginConversation(invocationContext.getAccessibleObject().getAnnotation(Begin.class).nested(),
	...
	private void endConversationIfNecessary(InvocationContext<T> invocationContext, Object result) {
		...
		boolean simpleEnd = (isEndAnnotation && invocationContext.getAccessibleObject().getAnnotation(End.class).ifOutcome().length == 0)
		...
		if (simpleEnd) {
			if (result != null || invocationContext.getConstructor() != null
					|| (invocationContext.getMethod() != null && invocationContext.getMethod().getReturnType().equals(void.class))) //null outcome interpreted as redisplay
			{
