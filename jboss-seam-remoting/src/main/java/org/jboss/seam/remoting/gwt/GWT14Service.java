...
public class GWT14Service extends GWTService {
	...
	public void startup() throws Exception {
		try {
			...
			streamReaderConstructor = ServerSerializationStreamReader.class
					.getConstructor(new Class[] { ClassLoader.class, policyProviderClass });
			streamWriterConstructor = ServerSerializationStreamWriter.class.getConstructor(new Class[] { serializationPolicyClass });
	...
	public ServerSerializationStreamReader getStreamReader() {
		try {
			return (ServerSerializationStreamReader) streamReaderConstructor.newInstance(Thread.currentThread().getContextClassLoader(),
					null);
		} catch (Exception ex) {
