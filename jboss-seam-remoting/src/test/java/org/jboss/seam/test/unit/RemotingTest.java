...
public class RemotingTest {
	...
	public void testStringWrapper() throws Exception {
		...
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		wrapper.marshal(out);
		byte[] expected = ("<str>" + URLEncoder.encode(value, StringWrapper.DEFAULT_ENCODING) + "</str>").replace("+", "%20").getBytes();
	...
	*/
	...
	public void testBagWrapper() throws Exception {
		...
		byte[] expected = ("<bag><element><number>" + intValues[0] + "</number></element>" + "<element><number>" + intValues[1]
				+ "</number></element></bag>").getBytes();
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		wrapper.marshal(out);
		assertEquals(expected, out.toByteArray());
	...
	*/
	...
	public void testWrapperFactory() {
		...
		assert WrapperFactory.getInstance().getWrapperForObject(Boolean.TRUE) instanceof BooleanWrapper;
		...
		assert WrapperFactory.getInstance().getWrapperForObject(Character.valueOf('a')) instanceof StringWrapper;
