...
public class PageScopedUIIncludeTest {
	...
	public static Archive<?> createDeployment() {
		...
		return Deployments.realSeamDeployment().addClasses(Component.class, Component1.class, Component2.class)
				.addAsWebResource(new StringAsset("<html xmlns=\"http://www.w3.org/1999/xhtml\""
						+ " xmlns:h=\"http://java.sun.com/jsf/html\"" + " xmlns:f=\"http://java.sun.com/jsf/core\""
						+ " xmlns:ui=\"http://java.sun.com/jsf/facelets\">" + "<h:head></h:head>" + "<h:body>" + "<h:form id='controller'>"
						+ "<h:commandButton id='component1' action='#{viewController.component1}' value='Component 1' />"
						+ "<h:commandButton id='component2' action='#{viewController.component2}' value='Component 2' />" + "</h:form>"
						+ "<ui:include src='#{viewController.viewId}'/>" + "</h:body>" + "</html>"), "test.xhtml")
	...
	private static Asset createComponentXhtmlAsset(int i) {
		return new StringAsset("<ui:composition xmlns=\"http://www.w3.org/1999/xhtml\"" + " xmlns:ui=\"http://java.sun.com/jsf/facelets\""
				+ " xmlns:h=\"http://java.sun.com/jsf/html\">" + "<h3>Component " + i + "</h3>" + "<h:form id='form" + i + "'>"
				+ "<h:inputText id='input' value='#{component" + i + ".input}' />"
				+ "<h:commandButton id='save' value='Save' action='#{component" + i + ".save}' />" + "<h:outputText value='#{component" + i
				+ ".output}' />" + "</h:form>" + "</ui:composition>"); // Yay for Java String syntax!
	}
