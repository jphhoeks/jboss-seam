...
public class RestoreViewComponentAccessTest {
	...
	public static WebArchive createDeployment() {
		...
		WebArchive war = Deployments.realSeamDeployment().addClasses(SequenceAction.class);

		war.delete("WEB-INF/pages.xml");
		war.delete("WEB-INF/components.xml");

		war.addAsWebResource(new StringAsset("<html xmlns=\"http://www.w3.org/1999/xhtml\"" + " xmlns:h=\"http://java.sun.com/jsf/html\""
				+ " xmlns:f=\"http://java.sun.com/jsf/core\"" + " xmlns:s=\"http://jboss.org/schema/seam/taglib\""
				+ " xmlns:ui=\"http://java.sun.com/jsf/facelets\">" + "<h:head></h:head>" + "<h:body>" + "<h:form id='form'>"
				+ "<h:messages/>" + "<h:outputText id='output' value='Sequence: #{sequence.output}'/>"
				+ "<h:inputText id='input' value='#{sequence.input}'>" + "<f:validateLongRange minimum='#{sequence.minimum}' />"
				+ "</h:inputText>" + "<h:commandButton id='append' value='Append' action='#{sequence.append}'/>" + "</h:form>" + "</h:body>"
				+ "</html>"), "test.xhtml");

		war.addAsWebInfResource(new StringAsset("<pages xmlns=\"http://jboss.org/schema/seam/pages\""
				+ " xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">" + "<page view-id='/test.xhtml'>"
				+ "<begin-conversation join='true'/>" + "<navigation><redirect view-id='/test.xhtml'/></navigation>" + "</page></pages>"),
				"pages.xml");
