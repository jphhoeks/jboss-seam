...
public class ResteasyBootstrap {
	...
	protected Collection<Class<?>> findResources(AnnotationDeploymentHandler handler) {
		return findTypes(handler, application.isScanResources(), javax.ws.rs.Path.class.getName(), application.getResourceClassNames());
	}
	...
	protected Collection<Component> findSeamComponents() {
		...
		for (String applicationContextName : applicationContextNames) {
			if (applicationContextName.endsWith(".component")) {
				Component seamComponent = (Component) Component.getInstance(applicationContextName, ScopeType.APPLICATION);
	...
	protected void registerResources(Collection<Component> seamComponents, Collection<Class<?>> resourceClasses) {
		...
		for (Component seamComponent : seamComponents) {
			...
			case JAVA_BEAN:
				...
				if (resourceInterface == null && !seamComponent.getBeanClass().isAnnotationPresent(javax.ws.rs.Path.class)) {
					break;
				}
				...
				case SESSION:
					...
					break;
				default:
					throw new RuntimeException("Resource Seam component '" + seamComponent.getName() + "' must be scoped either "
	...
	protected void registerHomeQueryResources(Component seamComponent) {
		...
		if (path != null) {
			...
			log.debug("registering resource, configured ResourceHome/Query on path {1}, as Seam component: {0}", seamComponent.getName(),
					path);
	...
	protected void registerInterfaceSeamComponentResource(Component seamComponent, Class resourceInterface) {
		...
		ResourceFactory factory = new SeamResteasyResourceFactory(resourceInterface, seamComponent, getDispatcher().getProviderFactory());
	...
	protected void registerStatelessEJBResource(Class ejbImplementationClass) {
		...
		log.debug("registering resource, stateless EJB implementation {1}, as RESTEasy JNDI resource name: {0}", jndiName,
