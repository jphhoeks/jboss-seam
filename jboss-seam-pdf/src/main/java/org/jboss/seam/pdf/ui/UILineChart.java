...
public class UILineChart extends UICategoryChartBase {
	...
	public JFreeChart createChart(FacesContext context) {
		...
		if (!getIs3D()) {
			chart = ChartFactory.createLineChart(getTitle(), getDomainAxisLabel(), getRangeAxisLabel(), (CategoryDataset) dataset,
					plotOrientation(getOrientation()), getLegend(), false, false);
		} else {
			chart = ChartFactory.createLineChart3D(getTitle(), getDomainAxisLabel(), getRangeAxisLabel(), (CategoryDataset) dataset,
					plotOrientation(getOrientation()), getLegend(), false, false);
		}
