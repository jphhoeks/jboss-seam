	...
	*/
		...
		try {
			...
			while (answer.hasMore()) {
				...
				ctx.modifyAttributes(sr.getNameInNamespace(),
						new ModificationItem[] { new ModificationItem(DirContext.REPLACE_ATTRIBUTE, user) });
			}
			answer.close();
	...
	public boolean grantRole(String name, String role) {
		...
		try {
			...
			BasicAttribute roleAttrib = new BasicAttribute(getUserRoleAttribute(), getRoleAttributeIsDN() ? getRoleDN(role) : role);
			...
			if (roleAttrib != null) {
				...
				for (int i = roleAttrib.size() - 1; i >= 0; i--) {
					if (getRoleAttributeIsDN()) {
						Attributes attribs = ctx.getAttributes((String) roleAttrib.get(i), new String[] { getRoleNameAttribute() });
	...
	public List<String> listRoles() {
		...
		try {
			...
			NamingEnumeration answer = ctx.search(getRoleContextDN(), roleFilter.toString(), filterArgs, controls);
			while (answer.hasMore()) {
				SearchResult sr = (SearchResult) answer.next();
				Attributes attrs = sr.getAttributes();
				Attribute user = attrs.get(getRoleNameAttribute());
