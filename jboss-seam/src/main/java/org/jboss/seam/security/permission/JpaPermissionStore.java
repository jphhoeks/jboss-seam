...
public class JpaPermissionStore implements PermissionStore, Serializable {
	...
	protected void initProperties() {
		...
		if (rolePermissionClass != null) {
			...
			if (roleProperty.isSet()) {
				roleTargetProperty = new AnnotatedBeanProperty<PermissionTarget>(rolePermissionClass, PermissionTarget.class);
				roleActionProperty = new AnnotatedBeanProperty<PermissionAction>(rolePermissionClass, PermissionAction.class);
			}
		} else {
		...
		if (rolePermissionClass != null) {
			...
			throw new RuntimeException("Invalid userPermissionClass " + userPermissionClass.getName()
					+ " - no rolePermissionClass set and @PermissionDiscriminator annotation not found on " + "any Field or Method");
		}
	}
		...
		try {
			if (recipientIsRole) {
				if (rolePermissionClass != null) {
					...
					ActionSet actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(instance).toString());
			...
			List permissions = createPermissionQuery(target, null, recipient, recipientIsRole ? Discrimination.role : Discrimination.user)
					.getResultList();
			...
			if (permissions.isEmpty()) {
				...
				if (discriminatorProperty.isSet()) {
					...
					discriminatorProperty.setValue(instance, recipientIsRole ? discriminator.roleValue() : discriminator.userValue());
				}
			...
			ActionSet actionSet = metadata.createActionSet(target.getClass(), actionProperty.getValue(instance).toString());
	...
	protected Principal resolvePrincipal(Object principal, boolean isUser) {
		...
		if (principal instanceof String) {
			return isUser ? new SimplePrincipal((String) principal)
					: new Role((String) principal, identityStore == null ? false : identityStore.isRoleConditional((String) principal));
		}
	...
	*/
	...
	protected List<Permission> listPermissions(Object target, Set<Object> targets, String action) {
		...
		for (Object permission : userPermissions) {
			...
			if (targets != null) {
				...
				if (target != null) {
					actionSet = metadata.createActionSet(target.getClass(), actionProperty.getValue(permission).toString());
				}
			} else {
				actionSet = metadata.createActionSet(target.getClass(), actionProperty.getValue(permission).toString());
			}
		...
		if (rolePermissionClass != null) {
			permissionQuery = targets != null ? createPermissionQuery(null, targets, null, Discrimination.role)
			...
			for (Object permission : rolePermissions) {
				...
				if (targets != null) {
					...
					if (target != null) {
						actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(permission).toString());
					}
				} else {
					actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(permission).toString());
				}
	...
	private Principal lookupPrincipal(Map<String, Principal> cache, Object permission, boolean isUser) {
		Principal principal = resolvePrincipal(isUser ? userProperty.getValue(permission) : roleProperty.getValue(permission), isUser);
	...
	public void clearPermissions(Object target) {
		...
		em.createQuery("delete from " + userPermissionClass.getName() + " p where p." + targetProperty.getName() + " = :target")
				.setParameter("target", identifier).executeUpdate();

		if (rolePermissionClass != null) {
			em.createQuery("delete from " + rolePermissionClass.getName() + " p where p." + roleTargetProperty.getName() + " = :target")
					.setParameter("target", identifier).executeUpdate();
		}
	}
}
