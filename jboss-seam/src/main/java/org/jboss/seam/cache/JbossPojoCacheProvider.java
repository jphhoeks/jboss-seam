...
@Scope(APPLICATION)
@BypassInterceptors
@Install(value = false, precedence = BUILT_IN, classDependencies = { "org.jboss.cache.aop.PojoCache", "org.jgroups.MembershipListener",
...
public class JbossPojoCacheProvider extends AbstractJBossCacheProvider<PojoCache> {
	...
	public void put(String region, String key, Object object) {
		try {
			...
			throw new IllegalStateException(
					String.format("JBoss Cache throw exception when adding object for key %s to region %s", key, region), e);
		}
	}
	...
	public void remove(String region, String key) {
		try {
			...
			throw new IllegalStateException(
					String.format("JBoss Cache throw exception when removing object for key %s in region %s", key, region), e);
		}
