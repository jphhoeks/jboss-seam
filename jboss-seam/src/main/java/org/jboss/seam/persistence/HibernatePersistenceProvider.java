		...
		if (hibernateSearchPresent) {
			try {
				...
				try {
					...
					// Hibernate search < 4
					log.trace(
							"org.hibernate.search.Search.getFullTextSession(Session) not found, trying deprecated method name createFullTextSession");
				...
				try {
					...
					// Hibernate search < 4
					log.trace(
							"org.hibernate.search.jpa.getFullTextSession(EntityManager) not found, trying deprecated method name createFullTextEntityManager");
					FULL_TEXT_ENTITYMANAGER_CONSTRUCTOR = jpaSearchClass.getDeclaredMethod("createFullTextEntityManager",
							EntityManager.class);
				}
	...
	*/
		...
		if (FULL_TEXT_ENTITYMANAGER_PROXY_CLASS == null) {
			...
			try {
				return (EntityManager) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
						new Class[] { FULL_TEXT_ENTITYMANAGER_PROXY_CLASS }, new EntityManagerInvocationHandler(
								(EntityManager) FULL_TEXT_ENTITYMANAGER_CONSTRUCTOR.invoke(null, super.proxyEntityManager(entityManager))));
			} catch (Exception e) {
	...
	public static void checkVersion(Object value, Session session, Object oldVersion, Object version) {
		...
		if (!versionType.isEqual(oldVersion, version)) {
			throw new StaleStateException("current database version number does not match passivated version number old:" + oldVersion
					+ " version:" + version + " value:" + value.toString());
		}
	}
