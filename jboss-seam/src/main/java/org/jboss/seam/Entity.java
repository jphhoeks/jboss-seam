 ...
 * 
 * A class will be identified as an entity class if it has an @Entity
 * annotation.
 * 
 * @author Gavin King
 * 
...
public class Entity extends Model {
	...
	private void mergeAnnotationAndOrmXml(EjbEntityDescriptor descriptor) {
		...
		if (descriptor != null) {
			...
			this.identifierField = descriptor.getIdentifierFieldName() != null
					? Reflections.getField(getBeanClass(), descriptor.getIdentifierFieldName())
					: null;
			this.identifierGetter = descriptor.getIdentifierPropertyName() != null
					? Reflections.getGetterMethod(getBeanClass(), descriptor.getIdentifierPropertyName())
					: null;

			this.versionField = descriptor.getVersionFieldName() != null
					? Reflections.getField(getBeanClass(), descriptor.getVersionFieldName())
					: null;
			this.versionGetter = descriptor.getVersionPropertyName() != null
					? Reflections.getGetterMethod(getBeanClass(), descriptor.getVersionPropertyName())
					: null;
		}
		...
		if (descriptor == null || !descriptor.isMetaDataComplete()) {
			for (Class<?> clazz = getBeanClass(); clazz != Object.class; clazz = clazz.getSuperclass()) {
				...
				for (Method method : clazz.getDeclaredMethods()) {
					...
					if (identifierField == null && identifierGetter == null && method.isAnnotationPresent(Id.class)
				...
				if ((identifierGetter == null && identifierField == null) || (versionField == null && versionGetter == null)) {
					for (Field field : clazz.getDeclaredFields()) {
						if (identifierGetter == null && identifierField == null
	...
	private static String lookupName(Class<?> beanClass, EjbEntityDescriptor descriptor) {
		...
		} else if ((descriptor == null || !descriptor.isMetaDataComplete()) && beanClass.isAnnotationPresent(javax.persistence.Entity.class)
				&& !"".equals(beanClass.getAnnotation(javax.persistence.Entity.class).name())) {
			// Is a name specified?
