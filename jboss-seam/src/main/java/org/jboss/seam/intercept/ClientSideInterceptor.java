...
public class ClientSideInterceptor extends RootInterceptor implements MethodHandler {
	...
	private boolean sessionBeanReturnedThis(Object result) {
		return result == bean || (result != null && getComponent().getBeanClass().isAssignableFrom(result.getClass()));
	}
		...
		try {
			if (comp == null) {
				ProxyObject proxy = Component
						.createProxyFactory(ComponentType.STATEFUL_SESSION_BEAN, beanClass, Component.getBusinessInterfaces(beanClass))
