...
public class JavaBeanInterceptor extends RootInterceptor implements MethodHandler {
	...
	private boolean markDirty(Method method) {
		return !getComponent().getBeanClass().isAnnotationPresent(ReadOnly.class) && !method.isAnnotationPresent(ReadOnly.class);
	}
	...
	Object readResolve() {
		...
		try {
			if (comp == null) {
				ProxyObject proxy = Component
						.createProxyFactory(ComponentType.JAVA_BEAN, beanClass, Component.getBusinessInterfaces(beanClass))
