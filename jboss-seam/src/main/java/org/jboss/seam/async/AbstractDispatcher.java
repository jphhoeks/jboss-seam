...
public abstract class AbstractDispatcher<T, S extends Schedule> implements Dispatcher<T, S> {
	...
	protected Schedule createSchedule(InvocationContext invocation) {
		...
		if (dispatcherParameters.getIntervalCron() == null) {
			return new TimerSchedule(dispatcherParameters.getDuration(), dispatcherParameters.getExpiration(),
					dispatcherParameters.getIntervalDuration(), dispatcherParameters.getFinalExpiration());
		} else {
			return new CronSchedule(dispatcherParameters.getDuration(), dispatcherParameters.getExpiration(),
					dispatcherParameters.getIntervalCron(), dispatcherParameters.getFinalExpiration());
		}
	}
			...
			for (Annotation annotation : annotations) {
				if (annotation.annotationType().equals(Duration.class)) {
					if (invocation.getParameters()[i] instanceof Long) {
						...
						throw new IllegalArgumentException("@Duration on " + invocation.getTarget().getClass() + ":"
								+ invocation.getMethod().getName() + " must be a Long");
					}
					...
					if (invocation.getParameters()[i] instanceof Date) {
						...
						throw new IllegalArgumentException("@Expiration on " + invocation.getTarget().getClass() + ":"
								+ invocation.getMethod().getName() + " must be a java.util.Date");
					}
					...
					if (!(this instanceof QuartzDispatcher)) {
						...
						throw new IllegalArgumentException("@FinalExpiration on " + invocation.getTarget().getClass() + ":"
								+ invocation.getMethod().getName() + " must be a java.util.Date");
					}
					...
					if (!(this instanceof QuartzDispatcher)) {
						...
						throw new IllegalArgumentException("@IntervalCron on " + invocation.getTarget().getClass() + ":"
								+ invocation.getMethod().getName() + " must be a String");
					}
					...
					if (invocation.getParameters()[i] instanceof Long) {
						...
						throw new IllegalArgumentException("@IntervalDuration on " + invocation.getTarget().getClass() + ":"
								+ invocation.getMethod().getName() + " must be a Long");
					}
				}
			}
