...
public class Pages {
	...
	*/
	protected void selectDataModelRow(FacesContext facesContext) {
		String dataModelSelection = facesContext.getExternalContext().getRequestParameterMap().get("dataModelSelection");
	...
	*/
	...
	protected void noConversation() {
		...
		FacesMessages.instance().addFromResourceBundleOrDefault(StatusMessage.Severity.WARN, "org.jboss.seam.NoConversation",
	...
	*/
	...
	private void parseConversation(Element element, String name) {
		...
		NaturalConversationIdParameter param = new NaturalConversationIdParameter(name, element.attributeValue("parameter-name"),
	...
	*/
	private Page parsePage(Page page, Element element, String viewId) {
		...
		String description = optionalElement == null ? element.getTextTrim() : optionalElement.getTextTrim();
	...
	*/
	...
	private static void parseNavigationHandler(Element element, Rule rule) {
		...
		if (redirect != null) {
			...
			Severity severity = severityName == null ? FacesMessage.SEVERITY_INFO
					: getFacesMessageValuesMap().get(severityName.toUpperCase());
			rule.addNavigationHandler(new RedirectNavigationHandler(stringValueExpressionFor(viewId), stringValueExpressionFor(url), params,
					message, severity, control, includePageParams));
		}
