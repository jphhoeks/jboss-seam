...
public class Contexts {
	...
	*/
	static void destroy(Context context) {
		if (Events.exists()) {
			Events.instance().raiseEvent("org.jboss.seam.preDestroyContext." + context.getType());
		}
		...
		if (Events.exists()) {
			Events.instance().raiseEvent("org.jboss.seam.postDestroyContext." + context.getType());
		}
	}
	...
	static void flushAndDestroyContexts() {
		...
		if (isConversationContextActive()) {
			...
			if (isBusinessProcessContextActive()) {
				...
				boolean destroyBusinessProcessContext = Init.instance() != null
						&& (!Init.instance().isJbpmInstalled() || !BusinessProcess.instance().hasActiveProcess());
