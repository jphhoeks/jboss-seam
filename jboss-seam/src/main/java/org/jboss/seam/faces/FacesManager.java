...
public class FacesManager extends Manager {
	...
	*/
	...
	protected void storeConversationToViewRootIfNecessary() {
		...
		if (facesContext != null
				&& (FacesLifecycle.getPhaseId() == PhaseId.RENDER_RESPONSE || FacesLifecycle.getPhaseId() == PhaseId.RESTORE_VIEW)) {
	...
	protected String generateInitialConversationId() {
		...
		if (viewId != null) {
			return Pages.instance().getPage(viewId).getConversationIdParameter()
	...
	*/
	public void prepareBackswitch(FacesContext facesContext) {
		...
		org.jboss.seam.pageflow.Page page = Manager.instance().isLongRunningConversation() && Init.instance().isJbpmInstalled()
				&& Pageflow.instance().isInProcess() && Pageflow.instance().isStarted() ? Pageflow.instance().getPage() : null;
