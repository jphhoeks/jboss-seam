...
public class Initialization
{
   ...
   private File warRoot;
    ...
    public Initialization(ServletContext servletContext) {
        ...
        this.hotDeployDirectory =
                getRealFile(servletContext, HotDeploymentStrategy.DEFAULT_HOT_DEPLOYMENT_DIRECTORY_PATH);
    }
    ...
    private void initComponentsFromXmlDocuments() {
        ...
        for (String duplicateJarRegex : new HashSet<String>(new SeamDeploymentProperties(Thread.currentThread()
                .getContextClassLoader()).getPropertyValues(DUPLICATE_JARS_PATTERNS))) {
            ...
            if (nsInfo == null) {
                ...
                try {
                    ...
                    throw new RuntimeException("Error loading element " + elemName + " with component name " + name
                            + " and component class " + className, e);
                }
        ...
        for (Element action : actions) {
            ...
            if (execute == null) {
                ...
                if (actionExpression != null) {
                    log.warn("<action expression=\"" + actionExpression
                            + "\" /> has been deprecated, use <action execute=\"" + actionExpression + "\" /> instead");
    ...
    private void installFactoryFromXmlElement(Element factory) {
        ...
        if (method == null && value == null) {
            throw new IllegalArgumentException(
                    "must specify either method or value in <factory/> declaration for variable: " + name);
        }
   ...
   @SuppressWarnings("unchecked")
   private void installComponentFromXmlElement(Element component, String name1, String className,
        ...
        if (className != null) {
            ...
            ComponentDescriptor descriptor =
                    new ComponentDescriptor(name, clazz, scope, autoCreate, startup, startupDepends, jndiName,
    ...
    */
    private ComponentDescriptor getHighestPriorityDescriptor(String componentName) {
        TreeSet<ComponentDescriptor> currentSet =
                (TreeSet<ComponentDescriptor>) this.componentDescriptors.get(componentName);
        if (currentSet != null) {
    ...
    */
    private boolean isProperty(String namespaceURI, String attributeName) {
        return (namespaceURI == null || namespaceURI.length() == 0)
                && !this.nonPropertyAttributes.contains(attributeName);
    }
    ...
    public void redeploy(HttpServletRequest request, Init init) throws InterruptedException {
        ...
        if (!isHotDeployEnabled(init)) {
            ...
            final WarRootDeploymentStrategy warRootDeploymentStrategy =
                    new WarRootDeploymentStrategy(Thread.currentThread().getContextClassLoader(), this.warRoot,
                            this.servletContext, new File[] { this.warClassesDirectory, this.warLibDirectory,
                                    this.hotDeployDirectory });
            ...
            if (changed) {
                ...
                if (warRootDeploymentStrategy.getTimestamp() > init.getWarTimestamp()) {
                    ...
                    Pages pages =
                            (Pages) ServletLifecycle.getServletContext().getAttribute(
                                    Seam.getComponentName(Pages.class));
                    if (pages != null) {
    ...
    private HotDeploymentStrategy createHotDeployment(ClassLoader classLoader, boolean hotDeployEnabled) {
        if (isGroovyPresent()) {
            ...
            return HotDeploymentStrategy.createInstance("org.jboss.seam.deployment.GroovyHotDeploymentStrategy",
                    classLoader, this.hotDeployDirectory, this.servletContext, hotDeployEnabled);
        }
        ...
        if (stream != null) {
            try {
                ...
                if (root.getName().equals("components")) {
                    ...
                    installComponentFromXmlElement(root, root.attributeValue("name"),
                            classFilenameFromDescriptor(fileDescriptor.getName()), replacements);
                }
            } catch (Exception e) {
    ...
    private void installComponents(Init init) {
        ...
        for (FactoryDescriptor factoryDescriptor : this.factoryDescriptors) {
            if (factoryDescriptor.isValueBinding()) {
                ...
                init.addFactoryMethodExpression(factoryDescriptor.getName(), factoryDescriptor.getMethod(),
                        factoryDescriptor.getScope());
            }
        ...
        for (EventListenerDescriptor listenerDescriptor : this.eventListenerDescriptors.values()) {
            for (String expression : listenerDescriptor.getListenerMethodBindings()) {
                init.addObserverMethodExpression(listenerDescriptor.getType(), Expressions.instance()
    ...
    */
    protected void addComponent(ComponentDescriptor descriptor, Context context) {
        ...
        try {
            Component component =
                    new Component(descriptor.getComponentClass(), name, descriptor.getScope(), descriptor.isStartup(),
