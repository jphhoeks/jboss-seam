...
public class MessageBrokerManager {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		try {
			...
			broker = config.createBroker(servletConfig.getInitParameter("messageBrokerId"), Thread.currentThread().getContextClassLoader());
			...
			if (broker.getChannelIds() == null || !broker.getChannelIds().contains(SEAM_ENDPOINT)) {
				...
				broker.createEndpoint(SEAM_ENDPOINT, "http://{server.name}:{server.port}/{context.root}/messagebroker/seam-amf",
	...
	private void setInitServletContext(flex.messaging.MessageBroker broker, ServletContext ctx) throws Exception {
		Method setMethod = flex.messaging.MessageBroker.class.getDeclaredMethod("setInitServletContext", ServletContext.class);
