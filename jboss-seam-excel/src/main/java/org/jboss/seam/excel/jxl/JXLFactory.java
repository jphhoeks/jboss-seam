...
public class JXLFactory {
	...
	*/
	public static Alignment createAlignment(String alignment) {
		...
		try {
			...
			String message = Interpolator.instance().interpolate("Alignment {0} not supported, try {1}", alignment,
					getValidConstantsSuggestion(ALIGNMENT_CLASS_NAME));
	...
	*/
	private static ScriptStyle createScriptStyle(String scriptStyle) {
		...
		try {
			return scriptStyle == null ? ScriptStyle.NORMAL_SCRIPT
					: (ScriptStyle) getConstant(SCRIPT_STYLE_CLASS_NAME, scriptStyle.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Script style {0} not supported, try {1}", scriptStyle,
					getValidConstantsSuggestion(SCRIPT_STYLE_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	private static UnderlineStyle createUnderlineStyle(String underlineStyle) {
		...
		try {
			return underlineStyle == null ? UnderlineStyle.NO_UNDERLINE
					: (UnderlineStyle) getConstant(UNDERLINE_STYLE_CLASS_NAME, underlineStyle.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Underline style {0} not supported, try {1}", underlineStyle,
					getValidConstantsSuggestion(UNDERLINE_STYLE_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static Colour createColor(String color) {
		...
		try {
			...
			String message = Interpolator.instance().interpolate("Color {0} not supported, try {1}", color,
					getValidConstantsSuggestion(COLOR_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	public static Orientation createOrientation(String orientation) {
		...
		try {
			return orientation == null ? Orientation.HORIZONTAL
					: (Orientation) getConstant(ORIENTATION_CLASS_NAME, orientation.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Orientation {0} not supported, try {1}", orientation,
					getValidConstantsSuggestion(ORIENTATION_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static VerticalAlignment createVerticalAlignment(String verticalAlignment) {
		...
		try {
			return verticalAlignment == null ? VerticalAlignment.BOTTOM
					: (VerticalAlignment) getConstant(VERTICAL_ALIGNMENT_CLASS_NAME, verticalAlignment.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Verical alignment {0} not supported, try {1}", verticalAlignment,
					getValidConstantsSuggestion(VERTICAL_ALIGNMENT_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static Border createBorder(String border) {
		...
		try {
			...
			String message = Interpolator.instance().interpolate("Border {0} not supported, try {1}", border,
					getValidConstantsSuggestion(BORDER_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static BorderLineStyle createLineStyle(String borderLineStyle) {
		...
		try {
			return borderLineStyle == null ? BorderLineStyle.NONE
					: (BorderLineStyle) getConstant(BORDER_LINE_STYLE_CLASS_NAME, borderLineStyle.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Border line style {0} not supported, try {1}", borderLineStyle,
					getValidConstantsSuggestion(BORDER_LINE_STYLE_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static Pattern createPattern(String pattern) {
		...
		try {
			...
			String message = Interpolator.instance().interpolate("Pattern {0} not supported, try {1}", pattern,
					getValidConstantsSuggestion(PATTERN_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static PageOrientation createPageOrientation(String orientation) {
		...
		try {
			return orientation == null ? PageOrientation.LANDSCAPE
					: (PageOrientation) getConstant(PAGE_ORIENTATION_CLASS_NAME, orientation.toUpperCase());
		} catch (NoSuchFieldException e) {
			String message = Interpolator.instance().interpolate("Page orientation {0} not supported, try {1}", orientation,
					getValidConstantsSuggestion(PAGE_ORIENTATION_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
	...
	*/
	public static PaperSize createPaperSize(String paperSize) {
		...
		try {
			...
			String message = Interpolator.instance().interpolate("Page size {0} not supported, try {1}", paperSize,
					getValidConstantsSuggestion(PAPER_SIZE_CLASS_NAME));
			throw new ExcelWorkbookException(message, e);
		}
	}
