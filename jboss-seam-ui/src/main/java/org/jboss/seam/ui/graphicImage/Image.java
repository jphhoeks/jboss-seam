...
public class Image implements Serializable {
	...
	public enum Type {
		IMAGE_PNG("image/png", ".png", "PNG"), IMAGE_JPEG("image/jpeg", ".jpg", "JPEG", "image/jpg"), IMAGE_GIF("image/gif", ".gif",
				"GIF"), IMAGE_BMP("image/bmp", ".bmp", "BMP");
	...
	public Image adjustRatio(double desiredRatio, double precision) throws InterruptedException, IOException {
		...
		if (!isRatio(desiredRatio, precision)) {
			if (getRatio() > desiredRatio) {
				...
				BufferedImage newImage = new BufferedImage(getWidth(), (int) (getHeight() + stripHeight * 2), getImageType());
				...
				BufferedImage newImage = new BufferedImage((int) (getWidth() + stripWidth * 2), getHeight(), getImageType());
				Graphics2D graphics2D = createGraphics(newImage);
