...
public class ViewUrlBuilder extends UrlBuilder {
	...
	public void addParameter(UIParameter parameter) throws UnsupportedEncodingException {
		...
		if (parameter.getValue() != null
				&& !(name.equals(page.getConversationIdParameter().getParameterName()) && getParameters().containsKey(name))) {
	...
	public String getEncodedUrl() {
		FacesContext facesContext = FacesContext.getCurrentInstance();
		return facesContext.getApplication().getViewHandler().getBookmarkableURL(facesContext, this.viewId,
				getParametersAsMap(getParameters()), false);
	}
