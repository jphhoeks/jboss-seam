...
public abstract class CommandButtonParameterRendererBase extends RendererBase {
	...
	protected void doEncodeEnd(ResponseWriter writer, FacesContext context, UIComponent component) throws IOException {
		...
		if (getUtils().isCommandButton(actionComponent)) {
			...
			String functionBody = "{" + "if (document.getElementById)" + "{" + "var form = document.getElementById('" + formId + "');"
					+ "var input = document.createElement('input');" + "if (document.all)" + "{ " + // what follows should work with NN6 but doesn't in M14"
					"input.type = 'hidden';" + "input.name = '" + getParameterName(component) + "';" + "input.value = '"
					+ parameter.getValue() + "';" + "}" + "else if (document.getElementById) " + "{" + // so here is theNN6 workaround
					"input.setAttribute('type', 'hidden');" + "input.setAttribute('name', '" + getParameterName(component) + "');"
					+ "input.setAttribute('value', '" + parameter.getValue() + "');" + "}" + "form.appendChild(input);" + "return true;"
					+ "}" + "}";

			String functionName = "cp_" + actionComponent.getId();

			String functionCode = "var " + functionName + " = " + "new Function(\"event\", \"" + functionBody + "\");";
			...
			String functionRegister = "if (document.getElementById('" + actionComponent.getClientId(context) + "'))" + "{"
					+ "document.getElementById('" + actionComponent.getClientId(context) + "').onclick = new Function(\"event\", \""
					+ functionBody + "\");" + "}";
