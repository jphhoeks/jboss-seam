	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		try {
			...
			for (String beanName : beanFactory.getBeanDefinitionNames()) {
				...
				if (scope == ScopeType.UNSPECIFIED) {
					if (log.isDebugEnabled()) {
						log.debug("Discarding bean with scope UNSPECIFIED.  Spring will throw an error later: " + beanName);
					}
					continue;
				}
				...
				if (definition.getBeanClassName() == null) {
					if (log.isDebugEnabled()) {
						log.debug("Unable to create component for bean: " + beanName + ".  No class defined try seam:component instead.");
					}
					continue;
				}
